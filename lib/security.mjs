...
export function encryptBinary(consultationId, attachmentId, plaintext) {
  const iv = randomBytes(12);
  const cipher = createCipheriv("aes-256-gcm", encryptionKey(), iv);
  cipher.setAAD(Buffer.from(`${consultationId}:attachment:${attachmentId}`, "utf8"));
  const ciphertext = Buffer.concat([cipher.update(plaintext), cipher.final()]);
  return { ciphertext, iv, authenticationTag: cipher.getAuthTag() };
}

export function decryptBinary(consultationId, attachmentId, encrypted) {
  const decipher = createDecipheriv("aes-256-gcm", encryptionKey(), encrypted.encryption_iv);
  decipher.setAAD(Buffer.from(`${consultationId}:attachment:${attachmentId}`, "utf8"));
  decipher.setAuthTag(encrypted.authentication_tag);
