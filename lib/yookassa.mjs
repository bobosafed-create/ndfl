...
function paymentReturnUrl(consultationId, purpose = "consultation") {
  ...
  returnUrl.searchParams.set("payment", purpose === "tariff_upgrade" ? "upgrade-return" : "return");
...
export async function createYooKassaPayment({ consultationId, idempotencyKey, amountKopecks, tariff, purpose = "consultation" }) {
  const returnUrl = paymentReturnUrl(consultationId, purpose);
  ...
  const payload = {
    ...
    metadata: { consultation_id: consultationId, tariff_code: tariff.code, payment_purpose: purpose },
  };
